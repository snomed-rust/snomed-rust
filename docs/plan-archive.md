# Plan archive — Phases 0-7 (2026-08-02 .. 2026-08-05)

The per-increment design narrative for the eight phases that built this
workspace, moved verbatim out of [`plan.md`](../plan.md) when that file
outgrew the repository's 40 KB per-document budget for the fourth time.
`plan.md` keeps a summary of each phase, the current phases in full, and
the forward-looking sections.

This is where the *reasoning* lives: why a decision went the way it did,
what was tried first, and which sources settled an ambiguity. Read it when
"why is it like this" matters; read [`tasks.md`](../tasks.md) and its own
archive for what happened when, and [`spec/`](../spec/README.md) for what
the behavior normatively is.
...
- Refinements ✅ (basic subset): `focus : attributeId (= | !=) value`, with
  ...
  `MINUS` — the grammar doesn't define one), `value` any
  hierarchy-prefixed expression, evaluated against active **inferred**
  relationships (spec/07's convention). Concrete value comparisons and
  non-plain-concept attribute names were rejected with a clear error at
  this point, never silently ignored; both landed in later increments.
...
- Numeric and string concrete value comparisons (2026-08-04) ✅ —
  `attr <= #10`, `attr = "E10.9"` (spec/07's concrete domains, spec/10's
...
- Concept filter constraint, `{{ C active = ... }}` (2026-08-05) ✅ — the
  first slice of the `{{ }}` filter subsystem (concept/description/member
  constraints, each with several kinds — too large for one increment),
  scoped to `conceptFilterConstraint`'s `activeFilter`:
  ...
  Vec<ConceptFilterKind> }` — an enum from the start (then just
  `Active`), matching the `AttributeComparison` precedent so a new filter
  kind wouldn't force a second breaking change; it paid off immediately. New lexer tokens
  ...
  apply *before* a trailing `:` wraps the result in a refinement,
  matching the grammar's own layering — backwards would have been a real
  correctness bug, not a parse-order detail. `{{ D ... }}`/`{{ M ... }}`
  and a bare `{{ ... }}` were recognized and rejected by name at this
  point (description filters landed in Phase 9); filters after a
  parenthesized
...
- `$lookup`'s `normalForm`/`normalFormTerse` (2026-08-05) ✅ — closed a
  ...
  entry point, so computing it inside `lookup` per call would cost whole
  seconds against real content — flagged rather than picked around. `lookup` gained a breaking
...
- New crate `snomed-owl` ✅: a hand-written lexer + recursive-descent
  ...
  *which* OWL constructs SNOMED CT uses — that had to come from
  ...
  concept inclusion (GCI) axioms needed no special-casing: they fall out
  once `SubClassOf`'s `sub` field is a general `ClassExpression`.
  **A parser, not a reasoner** at this point: classification came later,
  in Phase 7. Eager (whole-string) tokenization, unlike `snomed-ecl`'s
  pull-based lexer — OWL's fully bracketed grammar has none of the
  context-sensitive error masking that motivated ECL's design. Wired into the `snomed`
...
- **Necessary normal form** (2026-08-04) ✅: `necessary_normal_form`
  ...
  the reference implementation in two ways at this point, both documented
  as conservative (never wrong, occasionally less-reduced) rather than
  silent: no property-chain/transitive-property redundancy elimination
  (the reference's second pass — implemented later, see below), and no
  union-group handling (moot — EL has no disjunction operator at all). 8 new tests, including the two
