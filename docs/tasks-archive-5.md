# Tasks archive 5 of 5 — 2026-08-21

The 2026-08-21 sitting, moved verbatim out of [`tasks.md`](../tasks.md) to
keep it inside the repository's 40 KB per-document budget: `#[non_exhaustive]`,
the `agents/` rename, the four spec-compliance gaps, concrete-value history,
`$lookup` concept model attributes, `{{ D ... }}` description filters, the
row-based fuzz targets and the order dependence they found, refset member
history, `definitionStatusId`, and a review pass.
