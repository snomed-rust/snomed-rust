...
| | |
|---|---|
| Version | **0.23.0**, released 2026-09-06 |
...
0.23.0 gives the ECL `{{ M ... }}` member filter constraint's
`memberFieldFilter` alternative its ninth column, `valueId` — the
second implemented outside the two map types
(`AttributeValueRefsetMember`). Purely additive: no public API removed
...
- **Maturity.** Version 0.23.0, first published in September 2026, one
