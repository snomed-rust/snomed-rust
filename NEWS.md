...
| | |
|---|---|
| Version | **0.22.0**, released 2026-09-05 |
...
0.22.0 gives the ECL `{{ M ... }}` member filter constraint's
`memberFieldFilter` alternative its eighth column, `targetComponentId`
— the first implemented outside the two map types
(`AssociationRefsetMember`). Purely additive: no public API removed or
...
- **Maturity.** Version 0.22.0, first published in September 2026, one
