...
| | |
|---|---|
| Version | **0.45.0**, released 2026-09-10 |
...
0.45.0 gives the ECL `{{ M ... }}` member filter constraint's
`memberFieldFilter` alternative its thirtieth and final
`MrcmAttributeDomainRefsetMember` column, `attributeInGroupCardinality`
— still the string-search shape, and `MrcmAttributeDomainRefsetMember`'s
sixth and last column. Sharing a row with
`domainId`/`ruleStrengthId`/`contentTypeId`/`grouped`/`attributeCardinality`,
so no new store-side row-set check was needed. Completes that type's
column coverage — the fourth refset type outside the two map types to
...
| 2026-09-05 | 0.21.0 — `{{ M ... }}`'s `memberFieldFilter`: `mapCategoryId`, completing `ExtendedMap`'s column coverage |
| 2026-09-05 | 0.22.0 — `{{ M ... }}`'s `memberFieldFilter`: `targetComponentId`, first column outside the two map types |
| 2026-09-06 | 0.23.0 — `{{ M ... }}`'s `memberFieldFilter`: `valueId`, second column outside the two map types |
...
| 2026-09-06 | 0.28.0 — `{{ M ... }}`'s `memberFieldFilter`: `attributeDescription`, seventh column outside the two map types |
...
- **Maturity.** Version 0.45.0, first published in September 2026, one
