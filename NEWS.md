...
| | |
|---|---|
| Version | **0.30.0**, released 2026-09-06 |
...
0.30.0 gives the ECL `{{ M ... }}` member filter constraint's
`memberFieldFilter` alternative its fifteenth column, `attributeOrder`
— `RefsetDescriptorRefsetMember`'s third and last column, completing
that refset type's column coverage. Purely additive: no public API
...
- **Maturity.** Version 0.30.0, first published in September 2026, one
