...
| | |
|---|---|
| Version | **0.21.0**, released 2026-09-05 |
...
0.21.0 gives the ECL `{{ M ... }}` member filter constraint's
`memberFieldFilter` alternative its seventh and last `ExtendedMap`
column, `mapCategoryId` — completing `ExtendedMapRefsetMember`'s column
coverage entirely. Purely additive: no public API removed or changed,
...
- **Maturity.** Version 0.21.0, first published in September 2026, one
