...
| | |
|---|---|
| Version | **0.29.0**, released 2026-09-06 |
...
0.29.0 gives the ECL `{{ M ... }}` member filter constraint's
`memberFieldFilter` alternative its fourteenth column, `attributeType`
— `RefsetDescriptorRefsetMember`'s second column, after
`attributeDescription`. Purely additive: no public API removed or
...
- **Maturity.** Version 0.29.0, first published in September 2026, one
