...
- A member filter's `memberFieldFilter` kind other than `mapTarget`/
  ...
  `parentDomain` (2026-09-07) — the sixth string-search-shape column,
  `MrcmDomainRefsetMember`'s second column (both columns share one
  row) — reuses `mapTarget`'s grammar verbatim, no new row-set check
  needed;
  ...
  verbatim, no new row-set check needed;
  `guideURL` (2026-09-08) — the eleventh string-search-shape column,
  `MrcmDomainRefsetMember`'s seventh and last column (all seven
  columns share one row) — reuses `mapTarget`'s grammar verbatim, no
  new row-set check needed, completing `MrcmDomainRefsetMember`'s
  column coverage — the third refset type outside the two map types,
  after `RefsetDescriptorRefsetMember` and `DescriptionTypeRefsetMember`,
  to reach it;
  `domainId` (2026-09-09) — the ninth concept-reference-shape column,
  and the tenth refset type outside the two map types
  (`MrcmAttributeDomainRefsetMember`) — reuses `correlationId`'s
  grammar verbatim, needing a genuinely new twelfth row-set check
  tested against `SnapshotStore::mrcm_attribute_domain_member_rows`
  ...
  already put one there for that type. `value` extended (2026-09-13)
  the same way, completing `MemberAnnotationRefsetMember`'s column
