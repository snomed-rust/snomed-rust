...
stable Rust release minus three**: if the current stable release is `1.N`, the
MSRV is `1.(N-3)`.

Unlike the rest of `spec/`, this document is not a distillation of a SNOMED
International specification; it is a project policy that governs the Rust
toolchain the code in this workspace may assume.

## The rule

- Let `1.N.0` be the latest stable Rust release published by the Rust project.
- The MSRV MUST be `1.(N-3).0`.
...
roughly 18 weeks (~4 months). That is deliberately short: it keeps recent
language and standard-library work available while still giving downstream
consumers, distributions, and CI images a season to catch up.

## Where the MSRV is recorded

| Location | Form |
|---|---|
| `Cargo.toml` (`[workspace.package]`) | `rust-version = "1.(N-3)"` |
| each `crates/*/Cargo.toml` | `rust-version.workspace = true` |
| `.github/workflows/ci.yml` | an `msrv` job pinning `dtolnay/rust-toolchain@1.(N-3)` |
...
When a new stable Rust release `1.N` appears, the MSRV becomes `1.(N-3)`
**in the same change** that observes the release:

1. Set `rust-version` in the root `Cargo.toml` to `1.(N-3)`.
2. Set the pinned toolchain in the CI `msrv` job to the same value.
3. Run `cargo +1.(N-3) check --all-targets --workspace` and fix anything that
...
be avoided. Lowering it below N-3 (to support an older consumer) is a design
...
MSRV is **1.95**. If stable has moved on since, this document is stale in its
