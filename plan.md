...
- **ECL `{{ M ... }}`'s `memberFieldFilter` kind** (a refset-type-specific
  ...
  *counts* against, per CLAUDE.md rule 3, so counts below are stated as
  ...
  - Three shapes, none free:
    ...
    2. *Full typed rows, inactive included, for all sixteen non-
       ...
       (`domainTemplateForPrecoordination`, `domainTemplateForPostcoordination`, …), so the
  ...
  **Decided 2026-09-03: option 2** (full typed rows, inactive included,
  ...
  `attributeType`/`descriptionFormat`/`domainId`/`ruleStrengthId`/`contentTypeId`/`typeId`
  ...
  `sourceEffectiveTime`/`targetEffectiveTime` the time
  ...
  `{{ M effectiveTime }}`'s shared-column filter already has, both
  columns sharing one `ModuleDependencyRefsetMember` row) — all
...
shapes too; `targetComponentId`/`order` both then extended to
...
increment needed no `snomed-store` change; `attributeType`
(2026-09-06) is `RefsetDescriptorRefsetMember`'s second column,
another genuinely new variant, but needing no new row-set check at
all — both columns share one row, so the existing
`refset_descriptor_member_rows` block just grew a second `TypedFields`
entry, the same "two fields, one row" shape
`targetComponentId`/`order` have; and `attributeOrder` (2026-09-06) is
...
columns now populate from the same row; `domainConstraint`
...
row; `domainTemplateForPrecoordination` (2026-09-08) is
...
share one row; `grouped` (2026-09-09) is
...
again no new row-set check since all four columns share one row; and
...
types to reach it; `sourceEffectiveTime` (2026-09-10) is
...
outside the two map types; `rangeConstraint` (2026-09-10) is
...
check since all four columns implemented so far share one row; and
`attributeRule` (2026-09-10) is `MrcmAttributeRangeRefsetMember`'s
...
— a fourteenth refset type outside the two map types; and `typeId`
(2026-09-12) is `ComponentAnnotationRefsetMember`'s second column,
back on the concept-reference shape, another genuinely new variant
(no implemented column shares this RF2 field name in this filter
kind), no new row-set check since it shares
`languageDialectCode`'s row — but its own dedicated
`TokenKind::TypeIdKeyword` parser arm, since `typeId` is already a
...
`spec/10-ecl-member-filters.md`, mirroring how
`spec/10-ecl-unimplemented.md` was split out of `10-ecl.md` earlier
— five files now, still one normative whole, rule numbers
unaffected since they all live in `10-ecl.md`. In
between, the
