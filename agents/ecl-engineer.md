...
most needing their own value-set grammar. Implementing all of it in one
increment isn't realistic, and it hasn't been: `{{ C ... }}` came first,
one filter kind per commit; `{{ D ... }}` followed the same way; `{{ M
... }}` picked up its first three shared-column kinds
(`moduleId`/`effectiveTime`/`active`) in one increment since they
reused `{{ C }}`'s own machinery, after the `SnapshotStore` change
spec/09 rule 4 required first, then after `^R` (2026-09-02, its own
store index). Its refset-type-specific `memberFieldFilter` kind needed
its own store-retention decision first (`plan.md`'s "Open decisions",
decided 2026-09-03 — all sixteen non-Simple/Language types), then one
column per increment from `mapTarget` (2026-09-03) onward — the first
nineteen columns (`mapTarget` through `guideURL`, completing
`MrcmDomainRefsetMember`'s column coverage on 2026-09-08) are archived
verbatim in
[`docs/ecl-engineer-narrative-archive.md`](../docs/ecl-engineer-narrative-archive.md)
now that this file outgrew its own 40 KB budget — read it for the
early cases the lessons below assume: reusing a shape versus reusing a
variant are different questions (same column *name* reusing a variant
needs no new row-set check; a *shape* reusing existing grammar still
needs a genuinely new variant unless the RF2 field name itself
recurs), several columns can share one row once a type's row-set check
exists, and every grammar shape (concept-reference, string-search,
numeric) generalizes the same way. `domainId` (2026-09-09),
...
in the store). `ruleStrengthId` (2026-09-09),
...
matches. `value` (2026-09-12),
`ComponentAnnotationRefsetMember`'s third and last column, back on the
string-search shape, no new row-set check — completing that type's
column coverage, the seventh refset type outside the two map types to
reach it. `typeId` extended (2026-09-13) to
...
`NumericFieldFilter` grammar, not a new one to design; `mapRule` and
