...
`correlationId`, `mapGroup`, `mapPriority`, and `mapRule` followed
immediately after, one or two fields each of two more
...
`NumericFieldFilter` grammar, not a new one to design; `mapRule` did the
same for `mapTarget`'s string shape, reusing `TermFilter`/`term_matches`
verbatim. With the store side now done for all sixteen types, every
