...
impl Parser {
    ...
    fn parse_member_filter_kind(&mut self) -> Result<MemberFilterKind, EclError> {
        match &self.peek().kind {
            ...
            TokenKind::Word(word) if word == "domainConstraint" => {
                self.advance()?;
                let negated = self.parse_boolean_comparison_operator()?;
                let values = self.parse_typed_search_term_set()?;
                Ok(MemberFilterKind::DomainConstraint(TermFilter {
                    negated,
                    values,
                }))
            }
            ...
            TokenKind::Word(word) if word == "proximalPrimitiveRefinement" => {
                self.advance()?;
                let negated = self.parse_boolean_comparison_operator()?;
                let values = self.parse_typed_search_term_set()?;
                Ok(MemberFilterKind::ProximalPrimitiveRefinement(TermFilter {
                    negated,
                    values,
                }))
            }
            TokenKind::Word(word) if word == "domainTemplateForPrecoordination" => {
                self.advance()?;
                let negated = self.parse_boolean_comparison_operator()?;
                let values = self.parse_typed_search_term_set()?;
                Ok(MemberFilterKind::DomainTemplateForPrecoordination(
            ...
            TokenKind::Word(word) if word == "guideURL" => {
                self.advance()?;
                let negated = self.parse_boolean_comparison_operator()?;
                let values = self.parse_typed_search_term_set()?;
                Ok(MemberFilterKind::GuideUrl(TermFilter { negated, values }))
            }
            TokenKind::Word(word) if word == "domainId" => {
                self.advance()?;
                let negated = self.parse_boolean_comparison_operator()?;
                let value = self.parse_sub_expression_constraint()?;
                Ok(MemberFilterKind::DomainId(ModuleFilter {
                    negated,
                    value: Box::new(value),
                }))
            }
            ...
            TokenKind::Word(word) if word == "contentTypeId" => {
                self.advance()?;
                let negated = self.parse_boolean_comparison_operator()?;
                let value = self.parse_sub_expression_constraint()?;
                Ok(MemberFilterKind::ContentTypeId(ModuleFilter {
                    negated,
                    value: Box::new(value),
                }))
            }
            ...
            TokenKind::Word(word) if word == "targetEffectiveTime" => {
                self.advance()?;
                let operator = self.parse_time_comparison_operator()?;
                let values = self.parse_time_value_set()?;
                Ok(MemberFilterKind::TargetEffectiveTime(EffectiveTimeFilter {
                    operator,
                    values,
                }))
            }
            ...
            TokenKind::Word(word) if word == "attributeRule" => {
                self.advance()?;
                let negated = self.parse_boolean_comparison_operator()?;
                let values = self.parse_typed_search_term_set()?;
                Ok(MemberFilterKind::AttributeRule(TermFilter {
                    negated,
                    values,
                }))
            }
...
mod tests {
    ...
    /// grammar. No other implemented column shares this RF2 field
    /// name, so it's a genuinely new variant.
    #[test]
    fn parses_member_filter_domain_constraint() {
        let EC::MemberFilter { filters, .. } =
            parse("^ 723592007 {{ M domainConstraint = \"<< 404684003\" }}").unwrap()
        else {
            panic!("expected a member filter");
        };
        assert_eq!(filters.len(), 1);
        let crate::ast::MemberFilterKind::DomainConstraint(crate::ast::TermFilter {
            negated,
            values,
        }) = &filters[0]
        else {
            panic!("expected a DomainConstraint filter, got {:?}", filters[0]);
        };
        assert!(!negated);
        assert_eq!(values.len(), 1);
    ...
    /// field name, so it's a genuinely new variant.
    #[test]
    fn parses_member_filter_proximal_primitive_refinement() {
        let EC::MemberFilter { filters, .. } = parse(
            "^ 723592007 {{ M proximalPrimitiveRefinement = \"{ 116676008 = 415582006 }\" }}",
        )
        .unwrap() else {
            panic!("expected a member filter");
        };
        assert_eq!(filters.len(), 1);
        let crate::ast::MemberFilterKind::ProximalPrimitiveRefinement(crate::ast::TermFilter {
            negated,
            values,
        }) = &filters[0]
        else {
            panic!(
                "expected a ProximalPrimitiveRefinement filter, got {:?}",
                filters[0]
            );
        };
        assert!(!negated);
        assert_eq!(values.len(), 1);
        assert_eq!(values[0].text, "{ 116676008 = 415582006 }");
        assert_eq!(values[0].search_type, crate::ast::SearchType::Match);
    }
    ...
    /// genuinely new variant.
    #[test]
    fn parses_member_filter_domain_template_for_precoordination() {
        let EC::MemberFilter { filters, .. } = parse(
            "^ 723592007 {{ M domainTemplateForPrecoordination = \"[[+id(<< 71388002)]]: [[0..*]] { [[0..1]] 405815000 = [[+id]] }\" }}",
        )
        .unwrap() else {
            panic!("expected a member filter");
        };
        assert_eq!(filters.len(), 1);
        let crate::ast::MemberFilterKind::DomainTemplateForPrecoordination(
            crate::ast::TermFilter { negated, values },
        ) = &filters[0]
        else {
            panic!(
                "expected a DomainTemplateForPrecoordination filter, got {:?}",
                filters[0]
            );
        };
        assert!(!negated);
    ...
    /// `guideURL` (spec/10 rule 18) — the twenty-fourth `memberFieldFilter`
    /// column, and `MrcmDomainRefsetMember`'s seventh and last column
    /// (after `domainConstraint`/`parentDomain`/
    ...
    /// String-search shape, reusing `mapTarget`/`domainConstraint`'s
    /// exact grammar. No other implemented column shares this RF2
    /// field name, so it's a genuinely new variant. Completes
    /// `MrcmDomainRefsetMember`'s column coverage.
    #[test]
    fn parses_member_filter_guide_url() {
        let EC::MemberFilter { filters, .. } =
            parse("^ 723592007 {{ M guideURL = \"http://snomed.org/dom71388002\" }}").unwrap()
        else {
            panic!("expected a member filter");
        };
        assert_eq!(filters.len(), 1);
        let crate::ast::MemberFilterKind::GuideUrl(crate::ast::TermFilter { negated, values }) =
            &filters[0]
        else {
            panic!("expected a GuideUrl filter, got {:?}", filters[0]);
        };
        assert!(!negated);
        assert_eq!(values.len(), 1);
        assert_eq!(values[0].text, "http://snomed.org/dom71388002");
        assert_eq!(values[0].search_type, crate::ast::SearchType::Match);
    }

    /// `domainId` (spec/10 rule 18) — the twenty-fifth `memberFieldFilter`
    /// column, and the first on `MrcmAttributeDomainRefsetMember`, a
    /// tenth refset type outside the two map types. Concept-reference
    /// shape, reusing `correlationId`/`descriptionFormat`'s exact
    /// grammar. No other implemented column shares this RF2 field
    /// name, so it's a genuinely new variant, and — since this is
    /// `MrcmAttributeDomainRefsetMember`'s first filterable column — a
    /// genuinely new row-set check too.
    #[test]
    fn parses_member_filter_domain_id() {
        let EC::MemberFilter { filters, .. } =
            parse("^ 723592007 {{ M domainId = 404684003 }}").unwrap()
        else {
            panic!("expected a member filter");
        };
        assert_eq!(filters.len(), 1);
        let crate::ast::MemberFilterKind::DomainId(crate::ast::ModuleFilter { negated, .. }) =
            &filters[0]
        else {
            panic!("expected a DomainId filter, got {:?}", filters[0]);
        };
        assert!(!negated);
    }
    ...
    /// exact grammar. No other implemented column shares this RF2
    /// field name, so it's a genuinely new variant, but — since all
    /// three columns share one row — no new row-set check.
    #[test]
    fn parses_member_filter_content_type_id() {
        let EC::MemberFilter { filters, .. } =
            parse("^ 723592007 {{ M contentTypeId = 723596005 }}").unwrap()
        else {
            panic!("expected a member filter");
        };
        assert_eq!(filters.len(), 1);
        let crate::ast::MemberFilterKind::ContentTypeId(crate::ast::ModuleFilter {
            negated, ..
        }) = &filters[0]
        else {
            panic!("expected a ContentTypeId filter, got {:?}", filters[0]);
        };
        assert!(!negated);
    }
    ...
    /// second and last column. Still the time shape, reusing
    /// `parse_time_comparison_operator`/`parse_time_value_set`
    /// verbatim — the same parsing `sourceEffectiveTime` already has.
    /// Completes `ModuleDependencyRefsetMember`'s column coverage: no
    /// new row-set check needed since both columns share one row.
    #[test]
    fn parses_member_filter_target_effective_time() {
        let EC::MemberFilter { filters, .. } =
            parse("^ 900000000000534007 {{ M targetEffectiveTime >= \"20240101\" }}").unwrap()
        else {
            panic!("expected a member filter");
        };
        assert_eq!(filters.len(), 1);
        let crate::ast::MemberFilterKind::TargetEffectiveTime(crate::ast::EffectiveTimeFilter {
            operator,
            values,
        }) = &filters[0]
        else {
            panic!(
                "expected a TargetEffectiveTime filter, got {:?}",
                filters[0]
            );
        };
        assert!(matches!(operator, crate::ast::TimeComparisonOp::Ge));
    ...
    /// `attributeRule` (spec/10 rule 18) — the thirty-fourth
    /// `memberFieldFilter` column, still the string-search shape,
    /// reusing `mapTarget`/`rangeConstraint`'s exact grammar.
    /// `MrcmAttributeRangeRefsetMember`'s second and last column — no
    /// new row-set check since all five columns implemented so far
    /// share one row. Completes `MrcmAttributeRangeRefsetMember`'s
    /// column coverage.
    #[test]
    fn parses_member_filter_attribute_rule() {
        let EC::MemberFilter { filters, .. } = parse(
            "^ 723562003 {{ M attributeRule = \"116680003 |Is a| = 404684003 |Clinical finding|\" }}",
        )
        .unwrap() else {
            panic!("expected a member filter");
        };
        assert_eq!(filters.len(), 1);
        let crate::ast::MemberFilterKind::AttributeRule(crate::ast::TermFilter { negated, values }) =
            &filters[0]
        else {
            panic!("expected an AttributeRule filter, got {:?}", filters[0]);
        };
        assert!(!negated);
        assert_eq!(values.len(), 1);
        assert_eq!(
            values[0].text,
            "116680003 |Is a| = 404684003 |Clinical finding|"
        );
        assert_eq!(values[0].search_type, crate::ast::SearchType::Match);
    }
    ...
    #[test]
    fn parses_member_filter_value() {
        let EC::MemberFilter { filters, .. } =
            parse("^ 1292992004 {{ M value = \"free text\" }}").unwrap()
        else {
            panic!("expected a member filter");
        };
        assert_eq!(filters.len(), 1);
        let crate::ast::MemberFilterKind::Value(crate::ast::TermFilter { negated, values }) =
            &filters[0]
        else {
            panic!("expected a Value filter, got {:?}", filters[0]);
        };
        assert!(!negated);
        assert_eq!(values.len(), 1);
        assert_eq!(values[0].text, "free text");
        assert_eq!(values[0].search_type, crate::ast::SearchType::Match);
    }
    ...
    /// A field name this crate doesn't recognize (`referencedMemberId`,
    /// say — `MemberAnnotationRefsetMember`'s own column, distinct
    /// from `targetComponentId`, still unimplemented) falls to the
    /// generic bucket, not `NotYetImplemented` — the same "genuinely
    /// unimplemented" distinction rule 9 draws elsewhere. Not `typeId`
    /// or `value` any more: both used to be good examples of a
    /// lexer-keyword collision (`typeId` is `TokenKind::TypeIdKeyword`,
    /// from `{{ D typeId = ... }}`) or were simply unimplemented, but
    /// `ComponentAnnotationRefsetMember`'s column coverage is complete
    /// now — every column that type has is a recognized
    /// `memberFieldFilter` kind. Kept as a cautionary note here: check
    /// any future placeholder column name against
    /// `spec/10-ecl-unimplemented.md` and the lexer's keyword list
    /// before picking it.
    #[test]
    fn rejects_an_unrecognized_member_field_filter_generically() {
        assert!(matches!(
            parse("^ 447562003 {{ M referencedMemberId = \"x\" }}"),
