...
column but `mapTarget`/`correlationId`/`mapGroup`/`mapPriority` (e.g.
`order`, `domainConstraint`) are rejected too, but currently with a
generic parse
