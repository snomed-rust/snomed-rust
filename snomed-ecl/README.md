...
| Member filter | `^ 447562003 {{ M active = true }}`, `^R 73211009 {{ M moduleId = 900000000000207008 }}`, `{{ M effectiveTime >= "20200101" }}`, `{{ M mapTarget = "I21.9" }}`, `{{ M mapRule = "TRUE" }}`, `{{ M mapAdvice = wild:"ALWAYS*" }}`, `{{ M owlExpression = "SubClassOf" }}` (string search, `mapTarget` on `SimpleMap`/`ExtendedMap` rows, `mapRule`/`mapAdvice` on `ExtendedMap` rows only, `owlExpression` on `OwlExpression` rows only), `{{ M correlationId = 116680003 }}`, `{{ M mapCategoryId = 116680003 }}` (concept reference, `ExtendedMap` rows only), `{{ M targetComponentId = 116680003 }}` (concept reference, `Association` rows only), `{{ M valueId = 116680003 }}` (concept reference, `AttributeValue` rows only), `{{ M mapGroup >= #1 }}`, `{{ M mapPriority = #2 }}`, `{{ M order = #2 }}` (numeric, `mapGroup`/`mapPriority` on `ExtendedMap` rows only, `order` on `OrderedComponent` rows only) — restricts `^`'s referenced components, or `^R`'s result refsets, to those with a member row matching every filter in the block; `moduleId`/`effectiveTime`/`active` read the **member row's own** columns (spec/08), not its component's; active-only unless the block writes an `active` filter; works after both `^` and `^R` |
