...
| Member filter | `^ 447562003 {{ M active = true }}`, `^R 73211009 {{ M moduleId = 900000000000207008 }}`, `{{ M effectiveTime >= "20200101" }}`, `{{ M mapTarget = "I21.9" }}`, `{{ M mapRule = "TRUE" }}`, `{{ M mapAdvice = wild:"ALWAYS*" }}`, `{{ M owlExpression = "SubClassOf" }}`, `{{ M domainConstraint = "<< 404684003" }}`, `{{ M parentDomain = "<< 138875005" }}`, `{{ M proximalPrimitiveConstraint = "<< 71388002" }}`, `{{ M proximalPrimitiveRefinement = "{ 116676008 = 415582006 }" }}`, `{{ M domainTemplateForPrecoordination = "[[+id]]: { 116680003 = [[+id]] }" }}`, `{{ M domainTemplateForPostcoordination = "[[+id]]: { 116680003 = [[+id]] }" }}`, `{{ M guideURL = "snomed.org" }}` (string search, `mapTarget` on `SimpleMap`/`ExtendedMap` rows, `mapRule`/`mapAdvice` on `ExtendedMap` rows only, `owlExpression` on `OwlExpression` rows only, `domainConstraint`/`parentDomain`/`proximalPrimitiveConstraint`/`proximalPrimitiveRefinement`/`domainTemplateForPrecoordination`/`domainTemplateForPostcoordination`/`guideURL` on `MrcmDomain` rows only, sharing a row — every column that type has), `{{ M correlationId = 116680003 }}`, `{{ M mapCategoryId = 116680003 }}` (concept reference, `ExtendedMap` rows only), `{{ M targetComponentId = 116680003 }}` (concept reference, `Association`/`OrderedAssociation` rows), `{{ M valueId = 116680003 }}` (concept reference, `AttributeValue` rows only), `{{ M mrcmRuleRefsetId = 116680003 }}` (concept reference, `MrcmModuleScope` rows only), `{{ M attributeDescription = 116680003 }}`, `{{ M attributeType = 116680003 }}` (concept reference, `RefsetDescriptor` rows only, all three of that type's columns on the same row), `{{ M descriptionFormat = 900000000000540000 }}` (concept reference, `DescriptionType` rows only), `{{ M domainId = 404684003 }}`, `{{ M ruleStrengthId = 723589008 }}`, `{{ M contentTypeId = 723596005 }}` (concept reference, `MrcmAttributeDomain` rows: all three columns on the same row; `ruleStrengthId`/`contentTypeId` also match `MrcmAttributeRange` rows' own pair of those columns, a distinct row, same RF2 field names, no new filter kind needed), `{{ M mapGroup >= #1 }}`, `{{ M mapPriority = #2 }}`, `{{ M order = #2 }}`, `{{ M attributeOrder = #3 }}`, `{{ M descriptionLength = #255 }}` (numeric, `mapGroup`/`mapPriority` on `ExtendedMap` rows only, `order` on `OrderedComponent`/`OrderedAssociation` rows, `attributeOrder` on `RefsetDescriptor` rows only, `descriptionLength` on `DescriptionType` rows only, sharing a row with `descriptionFormat`), `{{ M grouped = true }}` (boolean, `MrcmAttributeDomain` rows only, sharing a row with `domainId`/`ruleStrengthId`/`contentTypeId`), `{{ M attributeCardinality = "0..1" }}`, `{{ M attributeInGroupCardinality = "0..1" }}` (string search, `MrcmAttributeDomain` rows only, sharing a row with `domainId`/`ruleStrengthId`/`contentTypeId`/`grouped`, and with each other — all six columns on the same row), `{{ M sourceEffectiveTime >= "20240101" }}`, `{{ M targetEffectiveTime >= "20240101" }}` (time, `ModuleDependency` rows only — the time shape's only implemented columns, the same grammar and machinery `{{ M effectiveTime }}` has, matched against the row's own `sourceEffectiveTime`/`targetEffectiveTime` columns, sharing a row with each other — both columns that type has), `{{ M rangeConstraint = "<< 27113001" }}`, `{{ M attributeRule = "116680003 = 404684003" }}` (string search, `MrcmAttributeRange` rows only — its own two columns, sharing a row with the extended `ruleStrengthId`/`contentTypeId` — every column that type has), `{{ M languageDialectCode = "en-GB" }}` (string search, `ComponentAnnotation` rows' first column, extended to `MemberAnnotation` rows' own column of the same name — a distinct row, no new filter kind needed), `{{ M typeId = 1295447006 }}` (concept reference, `ComponentAnnotation` rows' second column, sharing a row with `languageDialectCode`, extended to `MemberAnnotation` rows' own column of the same name — no new row-set check, reusing the block `languageDialectCode` already put there), `{{ M value = "a free-text note" }}` (string search, `ComponentAnnotation` rows' third and last column, sharing that row too — every column that type has, extended to `MemberAnnotation` rows' own column of the same name too — no new row-set check, completing `MemberAnnotation`'s column coverage but for `referencedMemberId`) — restricts `^`'s referenced components, or `^R`'s result refsets, to those with a member row matching every filter in the block; `moduleId`/`effectiveTime`/`active` read the **member row's own** columns (spec/08), not its component's; active-only unless the block writes an `active` filter; works after both `^` and `^R` |
...
column but `mapTarget`/`correlationId`/`mapGroup`/`mapPriority`/
`mapRule`/`mapAdvice`/`mapCategoryId`/`targetComponentId`/`valueId`/
...
`guideURL`/`domainId`/`ruleStrengthId`/`contentTypeId`/`grouped`/`attributeCardinality`/`attributeInGroupCardinality`/`sourceEffectiveTime`/`targetEffectiveTime`/`rangeConstraint`/`attributeRule`/`languageDialectCode`/`typeId`/`value`
(e.g. `referencedMemberId`, `MemberAnnotation`'s own column, distinct
from `targetComponentId`) are rejected
too, but currently with a generic parse error rather than a named one
(spec/10 rule 9) — genuinely unimplemented constructs, not just missing a
label.
